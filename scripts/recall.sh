...
if [ -f ${expdir}/${ckpt}.pth.tar ]; then
    ...
    python src/test.py --data_path ${datadir}/Freda-formatting/ --candidate ${expdir}/${ckpt}.pth.tar --vocab_path ${datadir}/SpokenCOCO_vocab-threshold1.pkl --basename ${basename} --constituent_recall
fi 
