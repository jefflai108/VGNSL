...
OMP_NUM_THREADS=5 # limit the number of threads for CPU tensor operations. Also add it in front of the running command just in case. 
MKL_NUM_THREADS=5 
...
#feature=hubert2_cat_disc-82_0; feature_dim=1536
feature=$4
jason_feats=disc-81_spokencoco_preFeats_max_0.7_9_clsAttn
jason_feats=$5
discovery_type=attn # change to word later
datadir=data/SpokenCOCO
expdir=exp/spokencoco/unsup_${discovery_type}_discovery_${jason_feats}_whole_${feature}_embed${embed_size}_MLPcombineV3_lr${lr}_${basename} # mlp_combine_v3 + deeper_score
echo $expdir
