...
from utils import hubert_feature_extraction, setup_vg_hubert, vghubert_feature_extraction
...
class SummaryJsonReader(object):
    def __init__(self, data_summary_json, feature,
        ...
        if 'cat' in feature:
            self.init_hubert()
            self.upstream_vghubert = setup_vg_hubert(model_type=feature.split('_cat_')[1], snapshot='best', device=self.device)
        else:
    ...
    def write_utterance_to_h5(self, seg_embed_h5_obj, vghubert_layer_num=11):
        ...
        for image_key in tqdm(self.utt_image_key_list):
            ...
            for captions in captions_list: 
                ...
                if 'hubert2' in self.feature:
                    ...
                    assert hubert_repre.shape[0] == self.hubert_dim
                    #print(hubert_repre.shape, hubert_nframes)
                vghubert_repre, vghubert_nframes = vghubert_feature_extraction(wav_file, self.upstream_vghubert, layer=vghubert_layer_num, device=self.device)
                assert vghubert_repre.shape[0] == self.vghubert_dim
                #print(vghubert_repre.shape, vghubert_nframes) # (768, 264) 264
                #print(vghubert_layer_num, type(vghubert_repre)) # <class 'numpy.ndarray'> (768, 511) 511

                if 'cat' in self.feature:
                    assert hubert_nframes == vghubert_nframes
                    final_repre = np.concatenate((hubert_repre, vghubert_repre), axis=0)
    ...
    def combine_h5_files(self, all_lab_embed_file, all_lab_len_file, seg_embed_h5_obj):
        print('writing to h5 object')
...
def main(args):
    ...
    if args.data_split == 'val':
        ...
        if args.h5_format:
            seg_embed_h5_obj = h5py.File(op.join(args.output_dir, f'val_segment-{args.feature}_{args.layer_num}_embed-' + basename + '.hdf5'), "w")
            val_writer.write_utterance_to_h5(seg_embed_h5_obj, vghubert_layer_num=args.layer_num)
    ...
    if args.data_split == 'test':
        ...
        if args.h5_format:
            seg_embed_h5_obj = h5py.File(op.join(args.output_dir, f'test_segment-{args.feature}_{args.layer_num}_embed-' + basename + '.hdf5'), "w")
            test_writer.write_utterance_to_h5(seg_embed_h5_obj, vghubert_layer_num=args.layer_num)
            seg_embed_h5_obj.close()
        ...
        if args.parallelize: 
            if args.h5_format:
                ...
                for tmp_lab_id in range(args.num_labs): 
                    lab_seg_embed_h5_obj = op.join(args.output_dir, f'.train_segment-{args.feature}_{args.layer_num}_embed-' + basename + '-' + str(tmp_lab_id) + '.hdf5')
                    if op.exists(lab_seg_embed_h5_obj): 
                ...
                if parallelize_done: 
                    ...
                    train_writer.combine_h5_files(all_lab_embed_file, \
                        h5py.File(op.join(args.output_dir, f'train_segment-{args.feature}_{args.layer_num}_embed-' + basename + '.hdf5'), 'w'))
                    for lab_seg_embed_h5_obj in all_lab_embed_file: 
                        lab_seg_embed_h5_obj.close()
                else: 
                    print('write partial numpy arrays to lab file')
                    lab_seg_embed_h5_obj = op.join(args.output_dir, f'.train_segment-{args.feature}_{args.layer_num}_embed-' + basename + '-' + str(args.lab_id) + '.hdf5')
                    if op.exists(lab_seg_embed_h5_obj):
                        print(f'{lab_seg_embed_h5_obj} already exist. Skip feature extraction')
                    else: 
                        lab_seg_embed_h5_obj = h5py.File(lab_seg_embed_h5_obj, "w")
                        train_writer.write_utterance_to_h5(lab_seg_embed_h5_obj, vghubert_layer_num=args.layer_num)
                        lab_seg_embed_h5_obj.close()
        else: 
            if args.h5_format:
                seg_embed_h5_obj = h5py.File(op.join(args.output_dir, f'train_segment-{args.feature}_{args.layer_num}_embed-' + basename + '.hdf5'), "w")
                train_writer.write_utterance_to_h5(seg_embed_h5_obj, vghubert_layer_num=args.layer_num)
                seg_embed_h5_obj.close()
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--feature', '-f', type=str, default='disc-81', 
                        choices = ['hubert2_cat_disc-81', 'hubert2_cat_disc-82', 'disc-81', 'disc-82'])
