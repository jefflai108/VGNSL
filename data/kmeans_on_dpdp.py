...
def load_feature_and_apply_km(apply_kmeans, n_clusters, data_dir, data_split, feature):
    output_h5_fpth = os.path.join(data_dir, f'{data_split}-dpdp-{feature}-pred_seg_feat-km{n_clusters}-83k-5k-5k.hdf5')
    word_list_fpth = os.path.join(data_dir, f'{data_split}-dpdp-pred_attn_list-83k-5k-5k.npy')
    word_list_keys = list(np.load(word_list_fpth, allow_pickle=True)[0].keys())
    word_seg_feats = np.load(os.path.join(data_dir, f'{data_split}-dpdp-{feature}-pred_seg_feat-83k-5k-5k.npy'), allow_pickle=True)[0]
...
def load_feature_and_train_km(km_model, data_dir, feature, percentage=0.1):
    word_seg_feats = np.load(os.path.join(data_dir, f'train-dpdp-{feature}-pred_seg_feat-83k-5k-5k.npy'), allow_pickle=True)[0]
    logging.info(f'Loading feature')
...
def learn_kmeans(data_dir, feature, kmeans_dir, n_clusters, seed, percent, init, 
    ...
    km_path = os.path.join(kmeans_dir, f'train-dpdp_{feature}-pred_seg_feat-83k-5k-5k.km{n_clusters}')
