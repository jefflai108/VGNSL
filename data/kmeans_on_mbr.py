...
def load_feature_and_apply_km(apply_kmeans, n_clusters, data_dir, data_split, feature):
    output_h5_fpth = os.path.join(data_dir, f'{data_split}-mbr_104_1030_top10-disc-81_snapshot15_layer9-pred_seg_feat-km{n_clusters}-83k-5k-5k.hdf5')
    word_list_fpth = os.path.join(data_dir, f'{data_split}-mbr_104_1030_top10-pred_word_list-83k-5k-5k.npy')
    word_list_keys = list(np.load(word_list_fpth, allow_pickle=True)[0].keys())
    word_seg_feats = np.load(os.path.join(data_dir, f'{data_split}-mbr_104_1030_top10-disc-81_snapshot15_layer9-pred_seg_feat-83k-5k-5k.npy'), allow_pickle=True)[0]
    logging.info(f'Loading feature')
    ...
    for tmp_idx in tqdm(word_list_keys):
        tmp_feat = word_seg_feats[tmp_idx]
        lab = apply_kmeans(tmp_feat).tolist()
        feature_km_h5_obj.create_dataset(str(tmp_idx), data=lab)
    feature_km_h5_obj.close()

def load_feature_and_train_km(km_model, data_dir, feature, percentage=0.1):
    word_seg_feats = np.load(os.path.join(data_dir, 'train-mbr_104_1030_top10-disc-81_snapshot15_layer9-pred_seg_feat-83k-5k-5k.npy'), allow_pickle=True)[0]
    logging.info(f'Loading feature')
    max_frame = int(20e6 * percentage)
    ...
    cur_frame_num = 0
    for tmp_idx in tqdm(range(len(word_seg_feats))): 
        tmp_feat = word_seg_feats[tmp_idx]
...
def learn_kmeans(data_dir, feature, kmeans_dir, n_clusters, seed, percent, init, 
    ...
    km_path = os.path.join(kmeans_dir, f'train-mbr_104_1030_top10-disc-81_snapshot15_layer9-pred_seg_feat-83k-5k-5k.km{n_clusters}')
