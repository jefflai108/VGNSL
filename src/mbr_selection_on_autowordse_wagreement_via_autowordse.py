...
def run(pred_tree_dir_list, unsup_word_discovery_feats=None, unsup_word_discovery_feat_type=None, data_split='test'): 
    ...
    executor = submitit.AutoExecutor(folder="exp/spokencoco/submitit_logs/log_test2")
    executor.update_parameters(
        timeout_min=4320, slurm_partition='sm,2080,titanx,titan,1080,cpu',
        cpus_per_task=4, gpus_per_node=0, nodes=1, slurm_mem='20G',
        slurm_array_parallelism=2, exclude='sls-sm-1,sls-sm-6,sls-1080-2,sls-sm-5,sls-1080-4'
    )
    ...
    #run_expr = lambda x: mbr_selection(x, pairwise_autowordse_score_for_mbr)
    def run_expr(configs): 
        pred_tree_sample, word_bound = configs 
        def pairwise_autowordse_score_for_mbr(induced_tree_1, induced_tree_2):
            return sample_wise_autowordse(induced_tree_1, induced_tree_2, word_bound, word_bound)
        return mbr_selection(pred_tree_sample, key_function=pairwise_autowordse_score_for_mbr)
    ...
    for i in tqdm(range(len(ground_truth_trees))): 
        ...
        configs.append((pred_tree_samples, word_bounds[i]))
        if len(configs) == 1000: # SLS limit 
            jobs = executor.map_array(run_expr, configs)
            configs = []
    ...
    for i in tqdm(range(len(ground_truth_trees))):
        if ground_truth_trees[i] == 'MISMATCH' or len(word_bounds[i]) < 2: 
            continue 
        output = all_jobs[cnt].result()
        cnt += 1
        mbr_selected_tree = output['best_sample']
        
        np.save(os.path.join('exp/spokencoco/submitit_results/autoword_se_temp_pickles', str(cnt) + '.npy'), mbr_selected_tree)
    
def agg_submitit_results(unsup_word_discovery_feats=None, unsup_word_discovery_feat_type=None, data_split='test'):
    ...
    mbr_submitit_dir = 'exp/spokencoco/submitit_results/autoword_se_temp_pickles'
    ...
    for i in tqdm(range(len(ground_truth_trees))): 
        if ground_truth_trees[i] == 'MISMATCH' or len(word_bounds[i]) < 2: 
            continue 
        cnt += 1
        mbr_selected_tree = np.load(os.path.join(mbr_submitit_dir, str(cnt) + '.npy'))
        mbr_selected_tree = str(mbr_selected_tree)
...
if __name__ == '__main__':
    ...
    agg_submitit_results(unsup_word_discovery_feats = 'mbr_104_1030_top10', 
                         unsup_word_discovery_feat_type = 'attn')
    exit()
